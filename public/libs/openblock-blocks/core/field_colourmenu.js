...
goog.provide('Blockly.FieldColourMenu');
 ...
 * @constructor
 */
Blockly.FieldColourMenu = function (icons, selectedBgColor) {
  /** @type {object} */
  this.icons_ = icons;
  // Example:
  // [{src: '...', value: 'machine_value'}, ...]
  // First icon provides the default values.
  var defaultValue = icons[0].value;
  Blockly.FieldColourMenu.superClass_.constructor.call(this, defaultValue);
  this.addArgType('colourmenu');
  this.selectedBgColor_ = selectedBgColor;

};
goog.inherits(Blockly.FieldColourMenu, Blockly.Field);

/**
 * Construct a FieldColourMenu from a JSON arg object.
 * @param {!Object} element A JSON object with options.
 * @returns {!Blockly.FieldColourMenu} The new field instance.
 * @package
 * @nocollapse
 */
Blockly.FieldColourMenu.fromJson = function (element) {
  return new Blockly.FieldColourMenu(element['options']);
};
 ...
 * @const
 */
Blockly.FieldColourMenu.DROPDOWN_WIDTH = 176;
 ...
 * @private
 */
Blockly.FieldColourMenu.savedPrimary_ = null;

/**
 * Called when the field is placed on a block.
 * @param {Block} block The owning block.
 */
Blockly.FieldColourMenu.prototype.init = function (block) {
  if (this.fieldGroup_) {
  ...
  Blockly.FieldColourMenu.superClass_.init.call(this, block);
};
 ...
 * @const
 */
Blockly.FieldColourMenu.prototype.CURSOR = 'default';
 ...
 * @override
 */
Blockly.FieldColourMenu.prototype.setValue = function (newValue) {
 ...
 * @private
 */
Blockly.FieldColourMenu.prototype.setParentFieldImage = function (src) {
 ...
 */
Blockly.FieldColourMenu.prototype.getValue = function () {
  return this.value_;
};
 ...
 */
Blockly.FieldColourMenu.prototype.getSrcForValue = function (value) {
 ...
 * @private
 */
Blockly.FieldColourMenu.prototype.showEditor_ = function () {
  ...
  for (var i = 0, icon; icon = this.icons_[i]; i++) {
    ...
    if (icon.type == 'placeholder') {
      ...
      placeholder.style.width = (icon.width || WIDTH) + 'px';
      placeholder.style.height = (icon.height || HEIGHT) + 'px';
    ...
    button.style.width = (icon.width || WIDTH) + 'px';
    button.style.height = (icon.height || HEIGHT) + 'px';
    ...
    // button.style.position = 'realative';
    ...
    if (icon.type === 'svg') {
      ...
      buttonImg.src = icon.iconSrc;
      // buttonImg.style.width = (icon.width || WIDTH) + 'px';
      // buttonImg.style.height = (icon.height || HEIGHT) + 'px';
   
    } else {
      if (icon.backgroundColor === 'transparent') {
        buttonImg.style.backgroundColor = '#fff';
        buttonImg.classList.add('transparent');
      } else {
        buttonImg.style.backgroundColor = icon.backgroundColor;
      }
      buttonImg.style.height = '100%';
      buttonImg.style.width = '100%';
      buttonImg.style.borderRadius = '100%';
    }
  ...
  contentDiv.style.width = Blockly.FieldColourMenu.DROPDOWN_WIDTH + 'px';
 ...
 * Should be bound to the FieldColourMenu.
 * @param {Event} e DOM event for the click/touch
 * @private
 */
Blockly.FieldColourMenu.prototype.buttonClick_ = function (e) {
 ...
 */
Blockly.FieldColourMenu.prototype.onHide_ = function () {
...
Blockly.Field.register('field_colourmenu', Blockly.FieldColourMenu);
