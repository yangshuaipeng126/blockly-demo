...
Blockly.BlockSvg.prototype.renderCompute_ = function() {
  ...
  for (var i = 0, input; input = this.inputList[i]; i++) {
    ...
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      ...
      if (field instanceof Blockly.FieldColourMenu) {
        metrics.iconMenu = field;
      }
      if (field instanceof Blockly.FieldBlockDirectionMenu) {
        metrics.iconMenu = field;
      }
      if (field instanceof Blockly.FieldMotorSpeedMenu) {
        metrics.iconMenu = field;
      }
      if (field instanceof Blockly.FieldMoveSpeedMenu) {
        metrics.iconMenu = field;
      }
      if (field instanceof Blockly.FieldImageMenu) {
        metrics.iconMenu = field;
      }
      if (field instanceof Blockly.FieldHorizontalMatrix) {
        metrics.matrix = field;
      }
