...
goog.provide('Blockly.FieldImageMenu');
 ...
 * @constructor
 */
Blockly.FieldImageMenu = function (icons) {
  /** @type {object} */
  this.icons_ = icons;
  // Example:
  ...
  Blockly.FieldImageMenu.superClass_.constructor.call(this, defaultValue);
  this.addArgType('iconmenu');
  this.preview_ = null;
};
goog.inherits(Blockly.FieldImageMenu, Blockly.Field);

/**
 * Construct a FieldImageMenu from a JSON arg object.
 * @param {!Object} element A JSON object with options.
 * @returns {!Blockly.FieldImageMenu} The new field instance.
 * @package
 * @nocollapse
 */
Blockly.FieldImageMenu.fromJson = function (element) {
  return new Blockly.FieldImageMenu(element['options']);
};
 ...
 * @const
 */
Blockly.FieldImageMenu.DROPDOWN_WIDTH = 194;
 ...
 * @private
 */
Blockly.FieldImageMenu.savedPrimary_ = null;

/**
 * Called when the field is placed on a block.
 * @param {Block} block The owning block.
 */
Blockly.FieldImageMenu.prototype.init = function (block) {
  if (this.fieldGroup_) {
  ...
  Blockly.FieldImageMenu.superClass_.init.call(this, block);
};
 ...
 * @const
 */
Blockly.FieldImageMenu.prototype.CURSOR = 'default';
 ...
 * @override
 */
Blockly.FieldImageMenu.prototype.setValue = function (newValue) {
 ...
 * @private
 */
Blockly.FieldImageMenu.prototype.setParentFieldImage = function (src) {
 ...
 */
Blockly.FieldImageMenu.prototype.getValue = function () {
  return this.value_;
};
 ...
 */
Blockly.FieldImageMenu.prototype.getSrcForValue = function (value) {
 ...
 * @private
 */
Blockly.FieldImageMenu.prototype.showEditor_ = function () {
  ...
  container.setAttribute('class', 'lls-image-thumb-selector');
  const grid = document.createElement('div');
  grid.setAttribute('class', 'lls-image-thumb-selector__grid');

  this.preview_ = document.createElement('div');
  this.preview_.setAttribute('class', 'lls-image-thumb-selector__selected');
  this.preview_.style.borderColor = this.sourceBlock_.getColour();
  container.appendChild(this.preview_);
  ...
  for (var i = 0, icon; icon = this.icons_[i]; i++) {
    ...
    button.setAttribute('class', 'lls-image-thumb-selector__image');
    ...
    var buttonImg = document.createElement('div');
    buttonImg.className = 'lls-image-thumb-selector__image__inner'
    buttonImg.style.backgroundImage = `url(${icon.src})`;
    buttonImg.style.borderColor = this.sourceBlock_.getColourTertiary();
    buttonImg.style.color = this.sourceBlock_.getColourTertiary();
    ...
    if (icon.value == this.getValue()) {
      ...
      button.classList.add('lls-image-thumb-selector__image--selected')
      button.firstChild.style.borderColor = '#fff';
      this.preview_.style.backgroundImage = `url(${icon.src})`;
    }
    icon.el = buttonImg;
    grid.appendChild(button);
  }
  contentDiv.appendChild(container)
  contentDiv.style.width = Blockly.FieldImageMenu.DROPDOWN_WIDTH + 'px';
  contentDiv.style.maxHeight = '305px'
 ...
 * @private
 */
Blockly.FieldImageMenu.prototype.buttonClick_ = function (e) {
  var value = e.target.getAttribute('data-value');
  this.setValue(value);
  console.log(this, value);
  const borderColor = this.savedPrimary_;
  this.icons_.forEach(item => {
    item.el.style.borderColor = borderColor
    item.el.style.color = borderColor
    if (item.value === value) {
      this.preview_.style.backgroundImage = `url(${value !== 'random' ? item.src : Blockly.mainWorkspace.options.pathToMedia + "icons/random.png"})`
    }
    item.el.parentElement.classList.remove('lls-image-thumb-selector__image--selected')
  })
  e.target.parentElement.classList.add('lls-image-thumb-selector__image--selected')
 ...
 */
Blockly.FieldImageMenu.prototype.onHide_ = function () {
...
Blockly.Field.register('field_imagemenu', Blockly.FieldImageMenu);
